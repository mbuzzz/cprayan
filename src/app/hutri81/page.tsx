...
const bootLines = [
  ...
  "[WARN] stability = 42%",
  "[WARN] some_services may still be running",
  "[ OK ] critical_thinking.module",
];

const dashboard = [
  "",
  "  Indonesia OS (Beta) :: BETA :: v81.0.0",
  "  Build: 17.08.1945-beta+81     Status: BETA_RELEASE",
  "  Warning: This version may contain bugs. Use your voice.",
  "",
];
...
function commandOutput(command: string): string[] {
  ...
  if (c === "help" || c === "--help") return [
    ...
    "  scan                 scan --issues=2026",
    "  rm -rf 02            remove obsolete services (simulation)",
  ...
  if (c === "scan" || c === "scan --issues=2026") return [
    "scan --issues=2026",
    "[WARN] Pendidikan.service: FileNotFoundError",
    "[WARN] desain.md: NotResponsiveError",
    ...
    "[WARN] masalah_2026.log: UnresolvedIssues",
    "scan complete: 7 warnings, 1 critical",
  ];
...
export default function Hutri81Page() {
  ...
  async function shareComplaints() {
    ...
    if (navigator.share) await navigator.share({ title: "Keluh Kesah Nusantara", text: "Keluh kesah masyarakat — IndonesiaOS", url });
  ...
  function runCommand(event: FormEvent<HTMLFormElement>) {
    ...
    setHistory((current) => [...current, `rayan@nusantara:~$ ${value}`, ...commandOutput(value)]);
    setCommand("");
  }
  ...
  return (
    <main className="hutri-page">
      ...
      <section className="terminal-window" aria-label="Indonesia OS terminal">
        <header className="terminal-bar"><div className="window-dots" aria-hidden="true"><i /><i /><i /></div><span className="terminal-title">root@nusantara: ~/hutri81/indonesia_os.py</span><span className="terminal-status">[ BETA_RELEASE ]</span></header>
        <div className="terminal-body"><div className="terminal-grid"><div className="terminal-main">
          ...
          <div className="progress-wrap"><div className="progress-label"><span>boot(IndonesiaOS_Beta)</span><strong>{progress}%</strong></div><div className="progress-track"><div style={{ width: `${progress}%` }} /></div></div>
          {booted && <div className="hero-copy"><div className="flag-mark"><span /><span /></div><p className="eyebrow">const HUT_RI = new Date("1945-08-17");</p><h1>print(<br /><em>"INDONESIA_MERDEKA"</em>)</h1><p className="hero-message"># TODO: fix_all_bugs_before_production();<br /># kritik() == health_check();</p><div className="hero-command"><span className="prompt">root@nusantara:~#</span> raise SystemExit("CHANGE_REQUIRED") <b>// exit_code: 2026</b></div></div>}
        ...
        </div><aside className="system-panel"><div className="panel-heading">class IndonesiaOS(Beta):</div><dl><div><dt>country</dt><dd>"ID"</dd></div><div><dt>version</dt><dd>81.0.0</dd></div><div><dt>status</dt><dd className="red-value">BUGGY</dd></div><div><dt>stability</dt><dd className="red-value">0.42</dd></div><div><dt>uptime</dt><dd>81 * YEAR</dd></div><div><dt>root_access</dt><dd>False</dd></div></dl><div className="panel-heading commit-heading">git log --oneline</div><ul className="commit-list"><li>desain.md <b>// NOT_RESPONSIVE</b></li><li>masalah_2026.log <b>// UNRESOLVED</b></li><li>korupsi.vulnerability <b>// CRITICAL</b></li><li>MBG.targeting_bug <b>// MISDIRECTED</b></li><li>KDKMP.anomaly <b>// DETECTED</b></li></ul><div className="ascii-flag"><span /><span /></div><p className="panel-note"># kritik = health_check()<br /># TODO: ship_change()</p></aside></div></div>
        <footer className="terminal-footer"><span>PID=1945</span><span>encoding="UTF-8"</span><span>env="BETA"</span><span>© RAYAN.WEB.ID</span></footer>
