...
export default function MlbbStemdaPage() {
  ...
  return <main className="mlbb-page">
    ...
    <section className="bracket"><div className="bracket-head"><h2>Bracket pertandingan</h2><span>Babak aktif: {roundNames[currentRound - 1]} • skor hanya bisa diubah admin</span></div><div className="bracket-board">{roundSizes.map((size, roundIndex) => { const round = roundIndex + 1; const matches = getRoundMatches(round); return <div className={`round-column ${round === currentRound ? "active" : ""}`} key={round}><div className="round-title"><b>{roundNames[roundIndex]}</b><small>{round === currentRound ? "SEDANG BERJALAN" : round < currentRound ? "SELESAI" : "MENUNGGU"}</small></div><div className="round-matches">{Array.from({ length: size }, (_, slotIndex) => { const m = matches[slotIndex]; return <article className={`match ${m?.winner ? "done" : ""} ${!m ? "placeholder" : ""}`} key={m?.id ?? `placeholder-${round}-${slotIndex}`}><div className="match-meta"><span>MATCH {String(slotIndex + 1).padStart(2, "0")}</span>{m?.winner && <b>WIN</b>}</div><div className={`team ${m?.winner === m?.a?.id ? "winner" : ""}`}><span>{m?.a?.name ?? (m ? "MENUNGGU" : "Pemenang match")}</span>{m && admin && round === currentRound ? <input aria-label={`Skor ${m.a?.name}`} type="number" min="0" value={m.scoreA ?? ""} onChange={e => updateScore(m.id, "a", e.target.value)} /> : <strong>{m?.scoreA ?? "—"}</strong>}</div><div className={`team ${m?.winner === m?.b?.id ? "winner" : ""}`}><span>{m?.b?.name ?? (m ? "MENUNGGU" : "vs match berikutnya")}</span>{m && admin && round === currentRound ? <input aria-label={`Skor ${m.b?.name}`} type="number" min="0" value={m.scoreB ?? ""} onChange={e => updateScore(m.id, "b", e.target.value)} /> : <strong>{m?.scoreB ?? "—"}</strong>}</div></article>; })}</div></div>; })}</div></section>
